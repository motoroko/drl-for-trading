...
class PPO:
    """
    Implementasi Proximal Policy Optimization (PPO) yang diadaptasi untuk bekerja
    dengan Vectorized Environments (VecEnv) untuk pelatihan paralel.
    """
    def __init__(
        self,
        policy_net,
        ...
        self.batch_size = self.n_steps * self.n_envs // 4 # Ukuran mini-batch dinamis
    ...
    def compute_gae_advantages(self, rewards, dones, values, last_values):
        # rewards, dones, values sekarang berbentuk (n_steps, n_envs)
        # Contoh: (2048, 4)
        advantages = np.zeros_like(rewards, dtype=np.float64)
        
        # <--- REVISI: Inisialisasi last_advantage sebagai vektor nol ---
        ...
        for step in reversed(range(self.n_steps)):
            ...
            # Ambil flag 'done' untuk langkah saat ini (t)
            is_terminal = dones[step]

            # Hitung delta dan advantage untuk semua env secara bersamaan
            ...
            # Simpan advantage saat ini untuk iterasi berikutnya (t-1)
            last_advantage = advantages[step]
        
        # Hitung returns dari advantage dan value
    ...
    def train(self, total_timesteps=100000, log_interval=1):
        ...
        for update_count in range(1, total_updates + 1):
            ...
            dones_buf = np.zeros((self.n_steps, self.n_envs), dtype=np.float64)
            ...
            for step in range(self.n_steps):
                ...
                next_state, rewards, dones, infos = self.env.step(action_clamped.cpu().numpy())
            ...
            advantages, returns = self.compute_gae_advantages(rewards_buf, dones_buf, values_buf, last_values)
            ...
            if update_count % log_interval == 0:
                ...
                for category, items in log_values.items():
                    ...
                    for name, value in items.items():
                        if name in ['fps','iterations','time_elapsed','total_timesteps','n_updates']: formatted_value = f"{value:d}"
                        elif abs(value) > 100000 or (abs(value) < 0.001 and value != 0): formatted_value = f"{value:.5e}"
